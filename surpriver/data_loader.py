...
class DataEngine:
	...
	def get_data(self, symbol):
		...
		try:
			...
			# get crytpo price from Haasonline
			elif(self.DATA_SOURCE == 'haas'):
				# Haas clients doesn't like 60m as an interval
				if(self.DATA_GRANULARITY_MINUTES == 'haas'):
    					
				stock_prices = md().get_market_data(symbol=symbol, interval = interval)
				# ensure that stock prices contains some data, otherwise the pandas operations below could fail
				if len(stock_prices) == 0:
					return [], [], True
				# convert list to pandas dataframe
